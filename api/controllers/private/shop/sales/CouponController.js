...
module.exports = {
  ...
  history: function (req, res) {
    return res.view('private/shop/sales/coupon/history', req.data);
  },
  historyData: function (req, res) {
    var pageSize = parseInt(req.body.length);
    var start = parseInt(req.body.start);
    var page = start / pageSize + 1;
    var draw = parseInt(req.body.draw);
    ...
    Shop_member_coupon.count(where).exec(function (err, count) {
      if (!err && count > 0) {
        Shop_member_coupon.find(where)
          .sort(sort)
          .populate('memberId',{select:['id','nickname']})
          .paginate({page: page, limit: pageSize})
          .exec(function (err, list) {
            return res.json({
              "draw": draw,
