 ...
 */
var Alipay = require('../common/alipay/alipay').Alipay;
        ...
        if (o && o.pay_alipay_info && o.pay_alipay_info.alipay_parter &&o.pay_alipay_info.alipay_key &&o.pay_alipay_info.alipay_email) {
          try {
            alipay_wap =new Alipay({
              ...
              ,transport:'http' //访问模式,根据自己的服务器是否支持ssl访问，若支持请选择https；若不支持请选择http
              ,input_charset:'utf-8'//字符编码格式 目前支持 gbk 或 utf-8
              ,create_direct_pay_by_user_notify_url:'/public/shop/wap/alipay/order'//异步通知地址
              ,create_direct_pay_by_user_return_url:'/public/shop/wap/shoppay/payStatusAlipay'//同步通知地址
            });
            cb(null,alipay_wap);
          } catch (err) {
            sails.log.debug('AlipayService err::'+JSON.stringify(err));
        ...
        if (o && o.pay_alipay_info && o.pay_alipay_info.alipay_parter && o.pay_alipay_info.alipay_key && o.pay_alipay_info.alipay_email) {
          try {
            alipay_fefund = new Alipay({
              ...
              ,refund_fastpay_by_platform_pwd_notify_url:'/public/shop/pc/alipay/aliRefund'//同步通知地址
            });
            cb(null,alipay_fefund);
          } catch (err) {
            sails.log.debug('AlipayService err::'+JSON.stringify(err));
