  function returnUnique(arr,checkarr){
    ...
    for(var i = 0, elem; (elem = arr[i]) != null; i++){
      if(!hash[elem] && elem != null && checkarr.indexOf(elem) < 0){
        result.push(elem);
        hash[elem] = true;
      }
    }
  ...
  function initializeSpec(checkload){
    ...
    if(!checkload){
      if(selectLength == specLength){
        ...
        $('#xsj').html('¥'+xprice);
        $('#scj').html('¥'+xpriceMarket);
        $('#kcs').html('(库存'+xstock+unit+')');
        if(isLvP){
          $.post("/public/shop/pc/goods/getLvPrice",
          {
            lvid:lvid,
            dis_count:dis_count,
            productid:xid,
